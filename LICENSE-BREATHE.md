...
<!--
  ...
  This file is a TEMPLATE published by Oxy. Carrying it in a repository does
  not license that repository. A work is licensed under these terms only when
  they are placed in that work's own LICENSE file with the Parameters filled
  in.
...
| Parameter | Value |
| --- | --- |
| **Licensor** | `<LEGAL ENTITY NAME>` (`<JURISDICTION OF INCORPORATION>`, `<REGISTRATION NUMBER>`) |
...
| Commercial licensing contact | `<licensing@EXAMPLE>` |
...
> Built on `<THE SOFTWARE>` by `<LEGAL ENTITY NAME>`, <https://oxy.so>. Licensed under the Breathe License 1.0.
